...
#define STR0052 "E R R O S"
#define STR0053 "A L E R T A S"
#define STR0054 "Observação: Erros impedem o funcionamento do sistema!"
...
Local nTipo   := 1
Local aTipos  := {OemToAnsi(STR0007), OemToAnsi(STR0008), OemToAnsi(STR0009)} //"Dados do Usuario" # "Dados do Servidor" # "Servicos Meu RH"
...
DEFINE WIZARD oWizard TITLE OemToAnsi(STR0001) ; //"MEU RH - Teste de Servicos"
   ...
   @ 30,20 RADIO oTipo VAR nTipo ITEMS aTipos[1],aTipos[2],aTipos[3] SIZE 65,8 PIXEL OF oPanel
...
LOCAL cErr        := OemToAnsi(STR0020) //"Existe erro!"
...
If nRet == 0
   ...
   For nX := 1 To Len(cLines)
      ...
      //Reinicia a variavel para poder avaliar a proxma linha
...
For nX := 1 To Len(aLogG)
   ...
   If nErr > 0
      AutoGrLog("") 
      AutoGrLog("") 
      AutoGrLog( "*** " + OemToAnsi(STR0052) + " ***" )  //"E R R O S"
      For nY := 1 To nErr
         AutoGrLog("") 
         AutoGrLog( aErrG[nX, 1, nY, 1] )
      Next nY         
      AutoGrLog("") 
      AutoGrLog( OemToAnsi(STR0054) ) //"Observação: Erros impedem o funcionamento do sistema!"
   EndIf
